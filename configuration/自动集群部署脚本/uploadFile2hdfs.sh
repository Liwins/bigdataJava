...
#log_src_dir=/var/log/httpd/

#待上传文件存放的目录
log_toupload_dir=/home/hadoop/logs/toupload/

#log_toupload_dir=/home/hadoop/hdfslogupload/
...
do
if [[ "$fileName" == access.lo* ]]; then
# if [ "access.log" = "$fileName" ];then
date=`date +%Y_%m_%d_%H_%M_%S`
#将文件移动到待上传目录并重命名
#打印信息
echo "moving $log_src_dir$fileName to $log_toupload_dir"xxxxx_click_log_$fileName"$date"
cp $log_src_dir$fileName $log_toupload_dir"xxxxx_click_log_$fileName"$date
