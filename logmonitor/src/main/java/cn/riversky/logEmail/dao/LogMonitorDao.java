...
public class LogMonitorDao {
    ...
    public List<App> getAppList() {
        String sql = "SELECT `id`,`name`,`isOnline`,`typeId`,`userId`  FROM `monitor`.`log_monitor_app` WHERE isOnline =1";
    ...
    public List<User> getUserList() {
        String sql = "SELECT `id`,`name`,`mobile`,`email`,`isValid` FROM `monitor`.`log_monitor_user` WHERE isValid =1";
    ...
    public void saveRecord(Record record) {
        String sql = "INSERT INTO `monitor`.`log_monitor_rule_record`" +
